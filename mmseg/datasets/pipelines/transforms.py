...
class MyRandomCrop(object):
    ...
    def __call__(self, results):
        ...
        crop_bbox = self.get_crop_bbox(img)
        crop_y1, crop_y2, crop_x1, crop_x2 = crop_bbox
        while x1 < crop_x1 or x2 > crop_x2 or y1 < crop_y1 or y2 > crop_y2:
            # Repeat 10 times
            for _ in range(10):
                crop_bbox = self.get_crop_bbox(img)
                crop_y1, crop_y2, crop_x1, crop_x2 = crop_bbox
...
@PIPELINES.register_module()
class FlipOD(object):
    def __init__(self, OD_position_path):
        self.OD_position_path = OD_position_path
        self.OD_info = dict()
        with open(OD_position_path) as f:
            lines = f.readlines()
            for line in lines:
                item = line.strip().split(' ')
                self.OD_info[item[0]] = [int(i) for i in item[1:]]

    def __call__(self, results):
        filename = results['ori_filename'].split('.')[0]
        OD_position = self.OD_info[filename]
        if OD_position == 1:
            results['img'] = mmcv.imflip(
                results['img'], direction='horizontal')

            # flip segs
            for key in results.get('seg_fields', []):
                # use copy() to make numpy stride positive
                results[key] = mmcv.imflip(
                    results[key], direction=results['horizontal']).copy()

        return results

    def __repr__(self):
        return self.__class__.__name__ + f'(OD_position_path={self.OD_position_path})'